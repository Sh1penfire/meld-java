...
public class Draww {
    ...
    public static void drawChain(TextureRegion region, float x, float y, float endx, float endy, float wScale, float hScale, float drawRotation){
        ...
        //This is in WU
        float height = region.height/4 * hScale;
        float width = region.width/4f * wScale;

        //Distance converted back to WU
        float remainder = ((distance * 4) % region.height * hScale)/8;

        for (int i = 0; i < Math.floor((distance)/region.height * 4); i++) {
            tv1.trns(angleToEnd, distance - i * height - region.height/8 * hScale).add(x ,y);
        ...
        Draw.rect(region, x + tv1.x, y + tv1.y, width, remainder * 2, drawRotation + angleToEnd - 90);
    }
