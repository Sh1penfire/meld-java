...
public class FlexibleSizeJunction extends LiquidJunction {
    ...
    public class FlexibleBuild extends LiquidJunctionBuild{
        ...
        public Building getLiquidDestination(Building source, Liquid liquid, Position otherOffset){
            ...
            //Rotate dir too
            int rotatedDir = transportRotation(source, dir);
            ...
            if(next instanceof FlexibleBuild b){
                canSend = b.acceptDirection(source, this, rotatedDir);
                if(canSend) return b.getLiquidDestination(this, liquid, Tmp.v2.scl(1f/size));
            }

            //If next is null then attempt to pas fluid onto other next valid junction
            if(next == null || (!next.acceptLiquid(this, liquid) && !(next.block instanceof LiquidJunction)) && next != this){
                ...
                Point2 sidewaysDir = Geometry.d4(rotatedDir + 1);
                Point2 rotatedDirection = Geometry.d4(rotatedDir);
                ...
                for(int i = 0; i < size; i++){
                    ...
                    Building junction = Vars.world.build(tile.x + rotatedDirection.x * tolerancePlus + sidewaysDir.x * j, tile.y + rotatedDirection.y * tolerancePlus + sidewaysDir.y * j);

                    //If we find a junction, pass along the offset
                    if(junction instanceof FlexibleBuild b && b.acceptDirection(source, this, rotatedDir)){
            ...
            return canSend ? next.getLiquidDestination(this, liquid) : null;
        }
    }
}
